...
class RegistersOneController
{
    ...
    public function store()
    {
        ...
        $stmt = $db->prepare("INSERT INTO register_one (emp_id, table_number, is_deleted) VALUES (?, ?, 0)");
    ...
    public function update($id)
    {
        ...
        // Check register_one exists
        $checkStmt = $db->prepare("SELECT id FROM register_one WHERE id = ? AND is_deleted = 0");
        ...
        $stmt = $db->prepare("UPDATE register_one SET emp_id = ?, table_number = ? WHERE id = ? AND is_deleted = 0");
    ...
    public function destroy($id)
    {
        ...
        $db = Database::getConnection();
        $stmt = $db->prepare("UPDATE register_one SET is_deleted = 1 WHERE id = ?");
        $stmt->execute([$id]);
    ...
    public function bulkDelete()
    {
        ...
        $registersToDelete = Database::fetchAll(
            "SELECT id, emp_id FROM register_one WHERE id IN ($placeholders) AND is_deleted = 0",
            $ids
        );
        ...
        if (count($foundIds) > 0) {
            ...
            Database::query(
                "UPDATE register_one SET is_deleted = 1 WHERE id IN ($deletePlaceholders)",
                $foundIds
            );
        }
    ...
    public function exportData()
    {
        ...
        $unregisteredQuery = "SELECT e.id, e.emp_name, p.position_name, d.dept_name, division_obj.div_name,
                              ...
                              LEFT JOIN division division_obj ON d.div_id = division_obj.id
                              WHERE e.is_deleted = 0
                                AND e.id NOT IN (SELECT DISTINCT emp_id FROM register_one WHERE is_deleted = 0)
                              ORDER BY division_obj.id, d.id, e.emp_name";
    ...
    public function stats()
    {
        ...
        // Total registers
        $totalStmt = $db->query("SELECT COUNT(*) as total FROM register_one WHERE is_deleted = 0");
        $total = $totalStmt->fetch()['total'];
        
        // Today's registers
        $todayStmt = $db->query("SELECT COUNT(*) as today FROM register_one WHERE is_deleted = 0 AND DATE(sys_datetime) = CURDATE()");
        $today = $todayStmt->fetch()['today'];
        
        // This month's registers
        $monthStmt = $db->query("SELECT COUNT(*) as month FROM register_one WHERE is_deleted = 0 AND MONTH(sys_datetime) = MONTH(CURDATE()) AND YEAR(sys_datetime) = YEAR(CURDATE())");
